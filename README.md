...
79.  矩阵微积分方法：先正常求导，注意非交换性，再对结果取转置。求导法则：$dy=\mathrm{tr}(\frac{dy}{d\mathbf{X}}d\mathbf{X})$，$d({\mathbf{X}\otimes\mathbf{Y}})=(d\mathbf{X})\otimes\mathbf{Y}+\mathbf{X}\otimes(d\mathbf{Y})$，$d({\mathbf{X}\circ\mathbf{Y}})=(d\mathbf{X})\circ\mathbf{Y}+\mathbf{X}\circ(d\mathbf{Y})$，$d(\mathbf{X}^{-1})=-\mathbf{X}^{-1}(d\mathbf{X})\mathbf{X}^{-1}$，$d(\mathrm{tr}(\mathbf{X}))=\mathrm{tr}(d\mathbf{X}))$ ，$d(|\mathbf{X}|)=\mathrm{tr}(adj(\mathbf{X})d\mathbf{X})=|\mathbf{X}|\mathrm{tr}(\mathbf{X}^{-1}d\mathbf{X})$ ，$df(x,\mathbf{y},\mathbf{Z})=\frac{\partial f}{\partial x}dx+\frac{\partial f}{\partial \mathbf{y}}d\mathbf{y}+\mathrm{tr}(\frac{\partial f}{\partial \mathbf{Z}}d\mathbf{Z})$                   例子： $\dfrac{\partial (b^Tx)}{\partial x} =  (\dfrac{b^T dx}{dx})^T = b$ ,   $\dfrac{\partial (x^TAx)}{\partial x} =(\dfrac{d(x^T)Ax+x^TAdx}{dx})^T=(\dfrac{x^TA^Tdx+x^TAdx}{dx})^T= (A+A^T)x$ 
